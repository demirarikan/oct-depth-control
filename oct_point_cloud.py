        ...
        for x in range(x_dim):
            ...
            for seg_id in values:
                ...
                if first_occurrence.size > 0:
                    first_occurrences = np.vstack(
                        (first_occurrences, np.array([z, first_occurrence[0][0], x]))
                    )
...
def create_mesh_sphere(center, radius=3, color=[1.0, 0.0, 1.0]):
    """
...
def create_save_point_cloud(
    ...
    needle_tip_sphere = create_mesh_sphere(
        needle_tip_coords, radius=3, color=[1.0, 0.0, 1.0]
    )
    ascan_cylinder = create_mesh_cylinder(needle_tip_coords, radius=0.3, height=500)

    needle_colors = np.array(
        [
            [1, 0, 0]
            for _ in range(np.asarray(cleaned_needle_point_cloud.points).shape[0])
        ]
    )
    ...
    oct_point_cloud.points = o3d.utility.Vector3dVector(
        np.vstack((ilm_points, rpe_points))
    )
    oct_point_cloud.colors = o3d.utility.Vector3dVector(
        np.vstack((ilm_colors, rpe_colors))
    )
...
def find_clusters_with_needle_angle(
    needle_point_cloud, needle_angle, angle_tolerance=10, eps=5, min_points=10
):
    cluster_labels = np.array(
        needle_point_cloud.cluster_dbscan(
...
def create_cylinder_pcd(
    radius=4, height=250, number_of_points=400, euler_angles=np.array([0, 35, 0])
):
    ...
    pcd.colors = o3d.utility.Vector3dVector(
        np.array([[0, 1, 0] for _ in range(number_of_points)])
    )
    return pcd
...
def outlier_detection_needle_estimate(
    needle_point_cloud, needle_estimate_point_cloud, radius=5
):
    distances = needle_point_cloud.compute_point_cloud_distance(
        needle_estimate_point_cloud
    )
...
def inpaint_layers(ilm_depth_map, rpe_depth_map, debug=False):
    ...
    # inpaint
    inpaint_ilm = cv2.inpaint(
        ilm_depth_map.astype(np.float32), ilm_inpainting_mask, 3, cv2.INPAINT_NS
    )
    inpaint_rpe = cv2.inpaint(
        rpe_depth_map.astype(np.float32), rpe_inpainting_mask, 3, cv2.INPAINT_NS
    )
...
def inpaint_layer(volume):
    ...
    rec_mesh = o3d.geometry.TriangleMesh.create_from_point_cloud_ball_pivoting(
        layer_pcd, o3d.utility.DoubleVector(radii)
    )
