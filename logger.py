...
class Logger:
    def __init__(
        self, log_dir="debug_log", run_name=datetime.now().strftime("%Y%m%d-%H%M%S")
    ):
    ...
    def log_volume(self, volume):
        np.save(
            os.path.join(self.oct_volumes_dir, f"volume_{self.__image_count}.npy"),
            volume,
        )
    ...
    def log_seg_results(self, oct_volume, seg_volume):
        ...
        for idx, seg_mask in enumerate(seg_volume):
            ...
            cv2.imwrite(
                os.path.join(
                    self.seg_images_dir, f"vol_{self.__image_count}_img_{idx}.png"
                ),
                blended_image,
            )
    ...
    def log_result_oct(self, needle_tip_coords, current_depth):
        ...
        needle_tip_image = cv2.imread(
            os.path.join(
                self.seg_images_dir,
                f"vol_{self.__image_count}_img_{needle_tip_coords[0]}.png",
            )
        )
        ...
        cv2.imwrite(
            os.path.join(self.result_oct_dir, f"needle_tip_{self.__image_count}.png"),
            needle_tip_image,
        )
